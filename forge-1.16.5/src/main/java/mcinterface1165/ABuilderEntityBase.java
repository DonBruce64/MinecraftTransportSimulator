...
public abstract class ABuilderEntityBase extends Entity {
    protected static final DeferredRegister<EntityType<?>> ENTITIES_REG = DeferredRegister.create(ForgeRegistries.ENTITIES, InterfaceLoader.MODID);
    protected static EntityType<ABuilderEntityBase> ENTITY_TYPE;
    ...
    public void baseTick() {
        ...
        if (world.isClient) {
            ...
            if (!playersRequestingData.isEmpty()) {
                for (IWrapperPlayer player : playersRequestingData) {
                    ...
                    writeNbt(((WrapperNBT) data).compound);
        ...
        if (!loadedFromSavedNBT && lastLoadedNBT != null && !world.isClient) {
