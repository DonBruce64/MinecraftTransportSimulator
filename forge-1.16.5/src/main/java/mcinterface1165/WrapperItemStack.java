...
public class WrapperItemStack implements IWrapperItemStack {
    ...
    public boolean isCompleteMatch(IWrapperItemStack other) {
        ...
        return otherStack.isItemEqualIgnoreDamage(stack) && (otherStack.hasTag() ? otherStack.getTag().equals(stack.getTag()) : !stack.hasTag());
    }
    ...
    public boolean interactWith(EntityFluidTank tank, IWrapperPlayer player) {
        ...
        if (handler != null) {
            if (!player.isSneaking()) {
                ...
                FluidStack containedStack = new FluidStack(ForgeRegistries.FLUIDS.getValue(new Identifier(tank.getFluid())), (int) tank.getFluidLevel());
    ...
    public void setData(IWrapperNBT data) {
        stack.setTag(data != null ? ((WrapperNBT) data).compound : null);
    }
}