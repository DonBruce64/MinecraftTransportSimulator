	...
	public void updatePart(){
		...
		if(definition.propeller.isDynamicPitch && !(vehicle instanceof EntityVehicleG_Blimp)){
			if(vehicle.reverseThrust && currentPitch > -MIN_DYNAMIC_PITCH){
				...
				++currentPitch;
			}else if(connectedEngine.RPM < (PartEngine.getSafeRPMFromMax(connectedEngine.definition.engine.maxRPM) - 150) && currentPitch > MIN_DYNAMIC_PITCH){
				--currentPitch;
			}else if(connectedEngine.RPM > (PartEngine.getSafeRPMFromMax(connectedEngine.definition.engine.maxRPM) - 100) && currentPitch < definition.propeller.pitch){
				++currentPitch;
			}
		}
