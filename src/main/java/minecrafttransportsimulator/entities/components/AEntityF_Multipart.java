...
public abstract class AEntityF_Multipart<JSONDefinition extends AJSONPartProvider> extends AEntityE_Interactable<JSONDefinition> {
     ...
     */
    public final HashMap<AItemPart, List<APart>> partsByItem = new LinkedHashMap<>();
     ...
     */
    public final HashMap<AItemPart, APart> lastPrimaryPart = new LinkedHashMap<>();
     ...
     */
    public final List<APart> partsFromNBT = new ArrayList<>();
    ...
    public PartSeat getSeatForRider(IWrapperEntity rider) {
        return (PartSeat) getPartAtLocation(riderLocationMap.inverse().get(rider));
    }
    ...
    public IWrapperEntity getController() {
        for (IWrapperEntity rider : riderLocationMap.inverse().keySet()) {
    ...
    public APart addPartFromItem(AItemPart partItem, IWrapperPlayer playerAdding, IWrapperNBT partData, Point3D offset, boolean addedDuringConstruction) {
        ...
        if (getPartAtLocation(offset) == null) {
            ...
            if (partItem.isPartValidForPackDef(newPartDef, subName, true)) {
                ...
                List<APart> partsToCheck = new ArrayList<>();
        ...
        //If we're in construction, it goes in the NBT maps then we add a rider position if it's a seat.
        ...
        if (partToAdd != null) {
            if (addedDuringConstruction) {
                ...
                if (partToAdd instanceof PartSeat) {
                    rideableLocations.add(partToAdd.placementOffset);
                }
            } else {
                ...
                if (partToAdd.definition.parts != null) {
                    for (JSONPartDefinition subPartPack : partToAdd.definition.parts) {
                        //addedDuringConstruction is false at this point
                        addDefaultPart(partToAdd.getPackForSubPart(subPartPack), playerAdding, partToAdd.definition, false, !newPart);
                    }
                }
            }
        }
    ...
    public void addPart(APart part, boolean sendPacket) {
        ...
        if (partItem != null) {
            if (!partsByItem.containsKey(partItem)) {
                partsByItem.put(partItem, new ArrayList<>());
            }
        ...
        if (part instanceof PartSeat) {
            rideableLocations.add(part.placementOffset);
        }
    ...
    public void removePart(APart part, Iterator<APart> iterator) {
        if (parts.contains(part)) {
            ...
            if (riderLocationMap.containsKey(part.placementOffset)) {
                removeRider(riderLocationMap.get(part.placementOffset));
            }
        ...
        } else partsFromNBT.remove(part);

        //Remove a ride-able location.
        if (part instanceof PartSeat) {
            rideableLocations.remove(part.placementOffset);
        }
    ...
    public LinkedHashMap<Point3D, JSONPartDefinition> getAllPossiblePackParts() {
        LinkedHashMap<Point3D, JSONPartDefinition> partDefs = new LinkedHashMap<>();
    ...
    public JSONPartDefinition getPackDefForLocation(Point3D offset) {
        ...
        List<APart> allParts = new ArrayList<>();
    ...
    protected void sortBoxes() {
        ...
        for (APart part : parts) {
            ...
            if (world.isClient()) {
                ...
                if (part instanceof PartSeat) {
                    if (part.placementOffset.equals(riderLocationMap.inverse().get(clientPlayer))) {
                        continue;
                    }
                }
                ...
                if (!(riderLocationMap.inverse().containsKey(clientPlayer) && part instanceof PartSeat) && areVariablesBlocking(part.placementDefinition, clientPlayer)) {
                    continue;
                }
