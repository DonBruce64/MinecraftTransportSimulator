...
public final class PartSeat extends APart {
    ...
    public boolean interact(IWrapperPlayer player) {
        ...
        if (isActive) {
            if (!entityOn.locked || entityOn.equals(player.getEntityRiding())) {
                IWrapperEntity riderForSeat = entityOn.riderLocationMap.get(placementOffset);
    ...
    public LanguageEntry checkForRemoval() {
        if (entityOn.riderLocationMap.get(placementOffset) != null) {
     ...
     * In a nutshell, this gets the riders' orientation assuming a non-rotated seat.
     */
    public void getRiderInterpolatedOrientation(RotationMatrix store, double partialTicks) {
        store.interpolate(prevZeroReferenceOrientation, zeroReferenceOrientation, partialTicks);
    }
    ...
    public void setNextActiveGun() {
        ...
        if (activeGun == null) {
            IWrapperEntity rider = entityOn.riderLocationMap.get(placementOffset);
            for (AItemPart partItem : entityOn.partsByItem.keySet()) {
    ...
    public double getRawVariableValue(String variable, float partialTicks) {
        ...
        switch (variable) {
            case ("seat_occupied"):
                return riderForSeat != null ? 1 : 0;
            case ("seat_occupied_client"):
                return InterfaceManager.clientInterface.getClientPlayer().equals(riderForSeat) ? 1 : 0;
            case ("seat_rider_yaw"):
                return riderForSeat != null ? riderForSeat.getYaw() : 0;
            case ("seat_rider_pitch"):
                return riderForSeat != null ? riderForSeat.getPitch() : 0;
        }
