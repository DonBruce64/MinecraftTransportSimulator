...
public class ItemPartGroundDevicePontoon extends AItemPart{
	...
	public ItemPartGroundDevicePontoon(JSONPart definition){
		super(definition);
	}
	
	@Override
	public boolean isPartValidForPackDef(VehiclePart packPart){
		return packPart.minValue <= definition.pontoon.width && packPart.maxValue >= definition.pontoon.width ? super.isPartValidForPackDef(packPart) : false;
	}
	...
	public void addInformation(ItemStack stack, @Nullable World world, List<String> tooltipLines, ITooltipFlag flagIn){
		...
		tooltipLines.add(I18n.format("info.item.ground_device.diameter") + definition.pontoon.width);
		tooltipLines.add(I18n.format("info.item.ground_device.motivefriction") + 0);
		tooltipLines.add(I18n.format("info.item.ground_device.lateralfriction") + definition.pontoon.lateralFriction);
