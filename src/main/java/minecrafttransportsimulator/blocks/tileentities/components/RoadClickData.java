...
public class RoadClickData{
	...
	public RoadClickData(TileEntityRoad roadClicked, JSONLaneSector sectorClicked, boolean clickedStart, boolean curveStart){
		...
		if(roadClicked.definition.road.type.equals(RoadComponent.CORE_DYNAMIC)){
			...
			genRotation = (float) (sectorClicked.sectorStartAngle + roadClicked.orientation.y.rotation + 180);
			genPosition = roadClicked.orientation.net.rotatePoint(sectorClicked.sectorStartPos.copy().add(-0.5, 0, -0.5)).add(roadClicked.position);
			if(curveStart){
