...
public class JSONPart extends AJSONPartProvider {
    ...
    @JSONDescription("Properties for intractables.")
    ...
    public static class JSONPartGeneric {
        ...
        @JSONDescription("This is a list of animatedObjects that can be used to move this part based on the animation values. This movement applies before the movement defined in the part slot. This is primarily used to move the part based on properties of sub-parts, mostly for guns, but may be used for other things.")
    ...
    public static class JSONPartEngine {
        ...
        @ModifiableValue
        @JSONDescription("The max RPM for this engine. This is how fast the engine will try to go with no load at 100% throttle. The red-line value (max safe RPM) is lower than and auto-calculated from this value.")
        public int maxRPM;

        @ModifiableValue
        @JSONDescription("The max safe (redline) RPM for this engine. If left out, MTS will auto-calculate this value for you. Normally this is fine, but SOME folks may not like the math, so manually specifying it is an option.")
    ...
    public static class JSONPartGroundDevice {
        ...
        @JSONDescription("The object order to render for the treads. Allows rendering of patterned tread links with multiple objects in the same model, rendering in the defined sequence. Used only if isTread is true.")
