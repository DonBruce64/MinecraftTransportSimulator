...
public class JSONRoadComponent extends AJSONMultiModelProvider {
    ...
    public class JSONRoadGeneric {
        ...
        @JSONDescription("A list of lane sectors that define the paths for this road. These contain a start position, angle, and lanes. Each sector is basically an intersection, with a 4-way intersection having 4 sectors. The properties in a sector are used to determine what lanes can connect to the sector, and where these connections are.")
        public List<JSONLaneSector> sectors;

        @JSONRequired(dependentField = "type", dependentValues = {"CORE_STATIC"})
        @JSONDescription("A list of collision areas. This allows for collision to be applied in a multi-block format for this road. Normally just a single square entry, but may extend in the +Y direction for complex road components like bridges. Think of how you select ares with the World Edit mod.")
