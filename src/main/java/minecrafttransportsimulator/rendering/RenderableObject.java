...
public class RenderableObject {
    ...
    private static final int BUFFERS_PER_HOLOGRAPHIC_BOX = 2 * 6 * BUFFERS_PER_FACE;
    //12 lines per box.
     ...
     * is holographic or wireframe as defined by the final boolean.**/
    public RenderableObject(ColorRGB color, boolean holographic) {
        this("", holographic ? "mts:textures/rendering/holobox.png" : null, color, FloatBuffer.allocate(holographic ? BUFFERS_PER_HOLOGRAPHIC_BOX : BUFFERS_PER_WIREFRAME_BOX), false);
        if (holographic) {
            isTranslucent = true;
            alpha = 0.5F;
        } else {
