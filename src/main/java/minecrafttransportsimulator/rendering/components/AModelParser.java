...
public abstract class AModelParser{
	 ...
	 */
	public static <AnimationEntity extends AEntityC_Definable<?>> List<RenderableModelObject<AnimationEntity>> generateRenderables(AnimationEntity entity, String modelLocation, List<JSONAnimatedObject> animatedObjects){
		Map<String, Float[][]> parsedModel = parseModel(modelLocation);
		List<RenderableModelObject<AnimationEntity>> modelObjects = new ArrayList<RenderableModelObject<AnimationEntity>>();
		for(String parsedObjectNames : parsedModel.keySet()){
			JSONAnimatedObject definition = null;
			if(animatedObjects != null){
				for(JSONAnimatedObject testDef : animatedObjects){
					if(parsedObjectNames.equals(testDef.objectName)){
						definition = testDef;
						break;
					}
				}
			}
			modelObjects.add(new RenderableModelObject<AnimationEntity>(modelLocation, parsedObjectNames, definition, parsedModel.get(parsedObjectNames), entity));
		}
		return modelObjects;
	}
}
