...
public class GUIPaintGun extends AGUIBase{
	...
	public GUIPaintGun(EntityVehicleF_Physics vehicle, IWrapperPlayer player){
		...
		this.player = player;
		this.currentItem = PackParserSystem.getItem(vehicle.definition.packID, vehicle.definition.systemName, vehicle.currentSubName);
	}
	...
	public void setupComponents(int guiLeft, int guiTop){	
		...
		addButton(confirmButton = new GUIComponentButton(guiLeft + 99, guiTop + 167, 20, "", 20, true, 20, 20, 20, 196, getTextureWidth(), getTextureHeight()){
			@Override
			public void onClicked(){
				MasterLoader.networkInterface.sendToServer(new PacketVehicleColorChange(vehicle, currentItem));
			}
		});
	...
	private void updateNames(){
		...
		List<AItemPack<?>> packItems = PackParserSystem.getAllItemsForPack(vehicle.definition.packID);
		...
		if(currentItemIndex < packItems.size()){
			for(int i=currentItemIndex+1; i<packItems.size() && nextSubItem == null; ++i){
				if(packItems.get(i).definition.systemName.equals(currentItem.definition.systemName)){
					nextSubItem = (ItemVehicle) packItems.get(i);
					break;
				}
			}
		}
		...
		if(currentItemIndex > 0){
			for(int i=currentItemIndex-1; i>=0 && prevSubItem == null; --i){
				if(packItems.get(i).definition.systemName.equals(currentItem.definition.systemName)){
					prevSubItem = (ItemVehicle) packItems.get(i);
					break;
				}
			}
		}
		...
		String modelLocation = currentItem.definition.getModelLocation();
		String textureLocation = currentItem.definition.getTextureLocation(currentItem.subName);
		modelRender.modelDomain = vehicle.definition.packID;
		modelRender.modelLocation = modelLocation;
		modelRender.textureDomain = vehicle.definition.packID;
