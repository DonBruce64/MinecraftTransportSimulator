...
public class GUIPanelAircraft extends AGUIPanel<EntityVehicleF_Air>{
	...
	protected int setupEngineComponents(int guiLeft, int guiTop, int xOffset){
		...
		if (haveReverseThrustOption && vehicle.definition.plane.hasAutopilot) { //If we have both reverse AND Autopilot, render them side-by-side. otherwise just render one in the middle
