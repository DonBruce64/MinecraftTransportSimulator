    ...
    public void init(FMLConstructModEvent event) {
            ...
            for (AItemPack<?> item : PackParser.getAllItemsForPack(packID, true)) {
                if (item.autoGenerate()) {
                    ...
                    BuilderItem.ITEMS.register(item.getRegistrationName(), () -> {
                        Item.Properties itemProperties = new Item.Properties().setId(ResourceKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(InterfaceLoader.MODID, item.getRegistrationName())));
                ...
                if (item instanceof IItemBlock) {
                    ...
                    if (!blocksRegistred.contains(itemBlockBlock)) {
                        ...
                        BuilderBlock.BLOCKS.register(name, () -> {
                            ResourceKey<Block> blockKey = ResourceKey.create(Registries.BLOCK, Identifier.fromNamespaceAndPath(InterfaceLoader.MODID, name));
                            BuilderBlock wrapper;

                            if (itemBlockBlock instanceof ABlockBaseTileEntity) {
                                wrapper = new BuilderBlockTileEntity(itemBlockBlock, blockKey);
                                ...
                                wrapper = new BuilderBlock(itemBlockBlock, blockKey);
                            }
        ...
        creativeTabsRequired.forEach((tabID, tabItems) -> {
            CREATIVE_TABS.register(tabID, () -> {
                ...
                //ItemStack creation is deferred; item component holders are not bound during the register event.
                ...
                Supplier<ItemStack> iconSupplier = tabIconItem != null ? () -> new ItemStack(BuilderItem.itemMap.get(tabIconItem)) : () -> new ItemStack(BuilderItem.itemMap.get(tabItems.get((int) (System.currentTimeMillis() / 1000 % tabItems.size()))));
        ...
        for (int i = 0; i < BlockCollision.blockInstances.size(); ++i) {
            ...
            BuilderBlock.BLOCKS.register(name, () -> {
                ResourceKey<Block> blockKey = ResourceKey.create(Registries.BLOCK, Identifier.fromNamespaceAndPath(InterfaceLoader.MODID, name));
                BuilderBlock wrapper = new BuilderBlock(collisionBlock, blockKey);
        ...
        BuilderTileEntity.TE_TYPE = BuilderTileEntity.TILE_ENTITIES.register("builder_base", () -> new BlockEntityType<>(BuilderTileEntity::new, Set.of(normalBlocks.toArray(new BuilderBlock[0]))));
        BuilderTileEntityFluidTank.TE_TYPE2 = BuilderTileEntity.TILE_ENTITIES.register("builder_fluidtank", () -> new BlockEntityType<>(BuilderTileEntityFluidTank::new, Set.of(fluidBlocks.toArray(new BuilderBlock[0]))));
        BuilderTileEntityInventoryContainer.TE_TYPE2 = BuilderTileEntity.TILE_ENTITIES.register("builder_inventory", () -> new BlockEntityType<>(BuilderTileEntityInventoryContainer::new, Set.of(inventoryBlocks.toArray(new BuilderBlock[0]))));
        BuilderTileEntityEnergyCharger.TE_TYPE2 = BuilderTileEntity.TILE_ENTITIES.register("builder_charger", () -> new BlockEntityType<>(BuilderTileEntityEnergyCharger::new, Set.of(chargerBlocks.toArray(new BuilderBlock[0]))));

        //Init entities.
        BuilderEntityExisting.E_TYPE2 = ABuilderEntityBase.ENTITIES.register("builder_existing", () -> EntityType.Builder.<BuilderEntityExisting>of(BuilderEntityExisting::new, MobCategory.MISC).sized(0.05F, 0.05F).clientTrackingRange(32 * 16).updateInterval(5).build(ResourceKey.create(Registries.ENTITY_TYPE, Identifier.fromNamespaceAndPath(InterfaceLoader.MODID, "builder_existing"))));
        BuilderEntityLinkedSeat.E_TYPE3 = ABuilderEntityBase.ENTITIES.register("builder_seat", () -> EntityType.Builder.<BuilderEntityLinkedSeat>of(BuilderEntityLinkedSeat::new, MobCategory.MISC).sized(0.05F, 0.05F).clientTrackingRange(32 * 16).updateInterval(5).build(ResourceKey.create(Registries.ENTITY_TYPE, Identifier.fromNamespaceAndPath(InterfaceLoader.MODID, "builder_seat"))));
        BuilderEntityRenderForwarder.E_TYPE4 = ABuilderEntityBase.ENTITIES.register("builder_rendering", () -> EntityType.Builder.<BuilderEntityRenderForwarder>of(BuilderEntityRenderForwarder::new, MobCategory.MISC).sized(0.05F, 0.05F).clientTrackingRange(32 * 16).updateInterval(5).build(ResourceKey.create(Registries.ENTITY_TYPE, Identifier.fromNamespaceAndPath(InterfaceLoader.MODID, "builder_rendering"))));
