...
public class BuilderEntityRenderForwarder extends ABuilderEntityBase {
    ...
    public void baseTick() {
        ...
        if (playerFollowing != null && playerFollowing.level() == this.level() && playerFollowing.isAlive()) {
            ...
            String playerUUID = lastLoadedNBT.getStringOr("playerFollowing", "");
            playerFollowing = playerUUID.isEmpty() ? null : level().getPlayerInAnyDimension(UUID.fromString(playerUUID));
    ...
    public void saveWithoutId(ValueOutput output) {
        ...
        if (playerFollowing != null) {
            ...
            CompoundTag entityTag = lastLoadedNBT != null ? lastLoadedNBT.copy() : new CompoundTag();
            entityTag.putString("playerFollowing", playerFollowing.getUUID().toString());
