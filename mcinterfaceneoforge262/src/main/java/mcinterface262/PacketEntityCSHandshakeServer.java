...
public class PacketEntityCSHandshakeServer extends APacketBase {
    ...
    public void handle(AWrapperWorld world) {
        if (builderID.contains(",")) {
            ...
            if (tile != null) {
                tile.lastLoadedNBT = ((WrapperNBT) data).tag.getCompoundOrEmpty(ABuilderEntityBase.MTS_DATA_KEY);
            ...
            if (mcWrapper != null && mcWrapper.entity != null) {
                //Set last loaded NBT.
                ((ABuilderEntityBase) mcWrapper.entity).lastLoadedNBT = ((WrapperNBT) data).tag.getCompoundOrEmpty(ABuilderEntityBase.MTS_DATA_KEY);
