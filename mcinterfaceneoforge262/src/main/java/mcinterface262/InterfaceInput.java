...
public class InterfaceInput implements IInterfaceInput {
    //Common variables.
    private static final KeyMapping.Category CATEGORY = new KeyMapping.Category(Identifier.fromNamespaceAndPath(InterfaceLoader.MODID, "main"));
    ...
    public void initConfigKey() {
        configKey = new KeyMapping(LanguageSystem.GUI_MASTERCONFIG.getCurrentValue(), GLFW.GLFW_KEY_P, CATEGORY);
        importKey = new KeyMapping(LanguageSystem.GUI_IMPORT.getCurrentValue(), GLFW.GLFW_KEY_UNKNOWN, CATEGORY);
    }
    ...
    public static void onIVMovementInput(MovementInputUpdateEvent event) {
        if (InterfaceManager.clientInterface != null && ControlSystem.shouldSuppressDismount(InterfaceManager.clientInterface.getClientPlayer(), event.getInput().keyPresses.shift())) {
            ...
            event.getInput().keyPresses = new Input(keys.forward(), keys.backward(), keys.left(), keys.right(), keys.jump(), false, keys.sprint());
        }
    }
