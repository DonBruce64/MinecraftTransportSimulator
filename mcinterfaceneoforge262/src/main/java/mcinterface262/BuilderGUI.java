...
public class BuilderGUI extends Screen {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        for (AGUIComponent component : gui.components) {
            if (component instanceof GUIComponentTextBox) {
                ...
                if (textBox.focused && textBox.handleMouseClicked(event.button())) {
                    return true;
                }
            }
        }
        if (gui.onClick((int) event.x(), (int) event.y())) {
    ...
    /**
     * Override to disable the background blur/darkening that MC adds by default.
     * MTS handles its own GUI rendering, so the vanilla blur just gets in the way.
     * In 26.2 this is where the blur stratum and menu background are applied.
     */
    @Override
