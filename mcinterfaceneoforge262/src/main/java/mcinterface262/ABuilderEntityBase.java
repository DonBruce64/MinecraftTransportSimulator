...
public abstract class ABuilderEntityBase extends Entity {
    /**
     * Key under which all MTS entity data is nested in the vanilla entity tag.
     * 26.2's ValueInput/ValueOutput API no longer exposes the raw entity tag, so
     * all data this builder needs to defer-load is stored under this single key.
     **/
    public static final String MTS_DATA_KEY = "mts";
        ...
        if (level().isClientSide()) {
            ...
            if (!playersRequestingData.isEmpty()) {
                for (IWrapperPlayer player : playersRequestingData) {
                    ...
                    ((WrapperNBT) data).tag.merge(output.buildResult());
    ...
    public void load(ValueInput input) {
        ...
        lastLoadedNBT = input.read(MTS_DATA_KEY, CompoundTag.CODEC).orElse(null);
    }
    ...
    public void saveWithoutId(ValueOutput output) {
        ...
        if (lastLoadedNBT != null) {
            output.store(MTS_DATA_KEY, CompoundTag.CODEC, lastLoadedNBT);
        }
    }
