...
public abstract class ABuilderEntityBase extends Entity {
    ...
    protected static final String MTS_NBT_KEY = "mts_entity";
        ...
        if (level().isClientSide()) {
            ...
            if (!playersRequestingData.isEmpty()) {
                for (IWrapperPlayer player : playersRequestingData) {
                    ...
                    addAdditionalSaveData(tagOutput);
                    CompoundTag outerTag = tagOutput.buildResult();
                    IWrapperNBT data = new WrapperNBT(outerTag.getCompound(MTS_NBT_KEY).orElseGet(CompoundTag::new));
    ...
    @Override
    public net.minecraft.world.entity.@org.jspecify.annotations.Nullable InterpolationHandler getInterpolation() {
        return null;
    }
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        //If entity hasn't ticked yet, preserve the loaded NBT so it isn't lost on a quick save.
        if (lastLoadedNBT != null) {
            output.store(MTS_NBT_KEY, CompoundTag.CODEC, lastLoadedNBT);
        }
    }

    @Override
    protected void readAdditionalSaveData(ValueInput input) {
        //Read MTS-specific data stored under a nested compound.
        input.read(MTS_NBT_KEY, CompoundTag.CODEC).ifPresent(tag -> lastLoadedNBT = tag);
    }
