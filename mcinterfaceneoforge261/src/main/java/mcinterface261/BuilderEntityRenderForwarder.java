...
public class BuilderEntityRenderForwarder extends ABuilderEntityBase {
    ...
    public void baseTick() {
        ...
        if (playerFollowing != null && playerFollowing.level() == this.level() && playerFollowing.isAlive()) {
            ...
            playerFollowing = level().getPlayerByUUID(UUIDUtil.uuidFromIntArray(lastLoadedNBT.getIntArray("playerFollowing").orElse(new int[4])));
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        ...
        if (playerFollowing != null) {
            //Player valid, save it.
            CompoundTag mtsTag = new CompoundTag();
            mtsTag.putIntArray("playerFollowing", UUIDUtil.uuidToIntArray(playerFollowing.getUUID()));
