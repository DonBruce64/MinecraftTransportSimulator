...
public class PacketEntityCSHandshakeServer extends APacketBase {
    ...
    public void handle(AWrapperWorld world) {
        if (builderID.contains(",")) {
            ...
            if (tile != null) {
                //saveAdditional wraps TE data under "mts_tile"; unwrap it to match what loadAdditional sets.
                CompoundTag outerTag = ((WrapperNBT) data).tag;
                tile.lastLoadedNBT = outerTag.getCompound("mts_tile").orElse(outerTag);
