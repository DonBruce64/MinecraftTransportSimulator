...
public class BuilderEntityLinkedSeat extends ABuilderEntityBase {
    ...
    private int lastUpdateRiderTick = -1;
    ...
    public void baseTick() {
        ...
        if (entity != null) {
            ...
            if (lastLoadedNBT.contains("entityUUID")) {
                entityUUID = UUIDUtil.uuidFromIntArray(lastLoadedNBT.getIntArray("entityUUID").orElse(new int[4]));
    ...
    protected void positionRider(Entity pPassenger, Entity.MoveFunction pCallback) {
        if (this.hasPassenger(pPassenger)) {
            ...
            if (entity != null && entity.rider != null) {
                //Gate: only call updateRider() once per tick.
                //positionRider can be invoked a second time in the same tick via
                //teleportPassengers() when the seat entity receives a server teleport packet.
                //A second updateRider() call would overwrite prevRiderCameraPosition with
                //the current riderCameraPosition (same value), destroying 1P camera interpolation.
                if (tickCount != lastUpdateRiderTick) {
                    lastUpdateRiderTick = tickCount;
                    entity.updateRider();
                }
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        ...
        if (entity != null) {
            //Entity is valid, save UUID.
            CompoundTag mtsTag = new CompoundTag();
            mtsTag.putIntArray("entityUUID", UUIDUtil.uuidToIntArray(entity.uniqueUUID));
