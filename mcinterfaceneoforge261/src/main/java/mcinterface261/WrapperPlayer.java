...
public class WrapperPlayer extends WrapperEntity implements IWrapperPlayer {
    ...
    public boolean isOP() {
        ...
        return server == null || server.getPlayerList().isOp(new net.minecraft.server.players.NameAndId(player.getGameProfile())) || server.isSingleplayer();
    }

    @Override
    public void displayChatMessage(LanguageEntry language, Object... args) {
        if (Minecraft.getInstance().player != null) {
            Minecraft.getInstance().player.sendSystemMessage(Component.literal(String.format(language.getCurrentValue(), args)));
        }
    }
    ...
    public IWrapperInventory getInventory() {
        return new WrapperInventory(player.getInventory()) {
            @Override
            public int getSize() {
                return player.getInventory().getNonEquipmentItems().size();
            }
        };
    }
