...
public class BuilderEntityExisting extends ABuilderEntityBase {
        ...
        if (entity != null) {
            ...
            if (!entity.isValid) {
                ...
                if (entity instanceof AEntityE_Interactable) {
                    ...
                    if (entity instanceof EntityVehicleF_Physics && interactable.ticksExisted > 1 && interactable.ticksExisted % 20 == 0) {
                        EntityDimensions newDims = EntityDimensions.scalable((float) Math.max(interactable.encompassingBox.widthRadius * 2F, interactable.encompassingBox.depthRadius * 2F), (float) interactable.encompassingBox.heightRadius * 2F);
                        if (mutableDims.width() != newDims.width() || mutableDims.height() != newDims.height()) {
                            mutableDims = newDims;
                            refreshDimensions();
                            setBoundingBox(collisionBoxes);
                        }
            ...
            if (!loadedFromSavedNBT && loadFromSavedNBT) {
                ...
                try {
                    ...
                    //In MC 1.21.5+, CompoundTag.getString() returns Optional<String>.
                    //Must call .orElse("") to get the actual String for the map lookup.
                    String entityId = lastLoadedNBT.getString("entityid").orElse("");
                    IItemEntityFactory factory = entityMap.get(entityId);
                    if (factory == null) {
                        InterfaceManager.coreInterface.logError("No entity factory found for id '" + entityId + "' on builder.  Did a pack change?");
                        discard();
                        return;
                    }
                    entity = factory.restoreEntityFromData(worldWrapper, data);
    ...
    public boolean hurtServer(ServerLevel serverLevel, DamageSource source, float amount) {
        if (ConfigSystem.settings.damage.allowExternalDamage.value && entity instanceof AEntityF_Multipart) {
    ...
    protected void addAdditionalSaveData(ValueOutput output) {
        ...
        if (entity != null) {
            //Entity is valid, save it.
            //Also save the class ID so we know what to construct when MC loads this Entity back up.
            CompoundTag mtsTag = new CompoundTag();
            entity.save(new WrapperNBT(mtsTag));
            mtsTag.putString("entityid", entity.getClass().getSimpleName());
            output.store(MTS_NBT_KEY, CompoundTag.CODEC, mtsTag);
        }
    }
