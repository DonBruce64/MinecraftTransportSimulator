...
public class InterfaceInput implements IInterfaceInput {
    ...
    public void initConfigKey() {
        configKey = new KeyMapping(LanguageSystem.GUI_MASTERCONFIG.getCurrentValue(), GLFW.GLFW_KEY_P, Category.MISC);
        importKey = new KeyMapping(LanguageSystem.GUI_IMPORT.getCurrentValue(), GLFW.GLFW_KEY_UNKNOWN, Category.MISC);
    }
    ...
    public boolean isMouseButtonPressed(int mouseButton) {
        return GLFW.glfwGetMouseButton(Minecraft.getInstance().getWindow().handle(), mouseButton) == GLFW.GLFW_PRESS;
    }
    ...
    public static void onIVMovementInput(MovementInputUpdateEvent event) {
        if (InterfaceManager.clientInterface != null && ControlSystem.shouldSuppressDismount(InterfaceManager.clientInterface.getClientPlayer(), event.getInput().keyPresses.shift())) {
            event.getInput().keyPresses = new net.minecraft.world.entity.player.Input(event.getInput().keyPresses.forward(), event.getInput().keyPresses.backward(), event.getInput().keyPresses.left(), event.getInput().keyPresses.right(), event.getInput().keyPresses.jump(), false, event.getInput().keyPresses.sprint());
        }
    }

    @SubscribeEvent
    public static void onIVInteractionKeyMapping(InputEvent.InteractionKeyMappingTriggered event) {
        if (event.isUseItem() && InterfaceManager.clientInterface != null && shouldSuppressVanillaRightClick(InterfaceManager.clientInterface.getClientPlayer())) {
            event.setCanceled(true);
            event.setSwingHand(false);
        }
    }

    private static boolean shouldSuppressVanillaRightClick(IWrapperPlayer player) {
