...
public class InterfaceClient implements IInterfaceClient {
    ...
    public List<String> getAllLanguages() {
        return new ArrayList<>(Minecraft.getInstance().getLanguageManager().getLanguages().keySet());
    }
    ...
    public Map<String, String> getAllFluidNames() {
        ...
        for (Entry<ResourceKey<Fluid>, Fluid> entry : ForgeRegistries.FLUIDS.getEntries()) {
            fluidIDsToNames.put(entry.getKey().location().getPath(), new FluidStack(entry.getValue(), 1).getDisplayName().getString());
        }
    ...
    public float getMouseSensitivity() {
        return (float) Minecraft.getInstance().options.sensitivity().get().doubleValue();
    }
    ...
    public List<String> getTooltipLines(IWrapperItemStack stack) {
        ...
        List<Component> tooltipLines = ((WrapperItemStack) stack).stack.getTooltipLines(Minecraft.getInstance().player, Minecraft.getInstance().options.advancedItemTooltips ? TooltipFlag.ADVANCED : TooltipFlag.NORMAL);
