	...
	public void updatePart(){
		...
		if(state.running || state.esOn){
			...
			if(getRatioForCurrentGear() != 0 && vehicle.wheels.size() > 0){
				engineForce = (engineTargetRPM - RPM)/definition.engine.maxRPM*getRatioForCurrentGear()*vehicle.definition.car.axleRatio*(definition.engine.fuelConsumption+(definition.engine.superchargerFuelConsumption*(definition.engine.superchargerEfficiency*2)))*0.6F;
