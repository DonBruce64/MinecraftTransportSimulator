...
/**
 * Packet sent to pumps to pay for fluids.  This will remove an item
 ...
 * @author don_bruce
 */
public class PacketTileEntityFuelPumpPayment extends APacketEntityInteract<ATileEntityFuelPump, IWrapperPlayer> {
    private final int slotClicked;
    private final int amountChangedTo;

    public PacketTileEntityFuelPumpPayment(ATileEntityFuelPump pump, IWrapperPlayer player, int slotClicked, int amountChangedTo) {
        super(pump, player);
        this.slotClicked = slotClicked;
        this.amountChangedTo = amountChangedTo;
    }

    public PacketTileEntityFuelPumpPayment(ATileEntityFuelPump pump, IWrapperPlayer player, int slotClicked) {
        super(pump, player);
        this.slotClicked = slotClicked;
        this.amountChangedTo = -1;
    }

    public PacketTileEntityFuelPumpPayment(ByteBuf buf) {
        super(buf);
