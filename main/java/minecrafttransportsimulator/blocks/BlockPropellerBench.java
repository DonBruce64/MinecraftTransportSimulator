		...
		if(player.getDistance(x, y, z) < 5){
			...
			if(!world.isRemote){
				if(bench.getPropellerOnBench() != null){
					...
					MTS.MFSNet.sendTo(new ChatPacket(PlayerHelper.getTranslatedText("interact.failure.propellerbenchworking")), (EntityPlayerMP) player);
				}
			}else{
				if(!bench.isRunning() && bench.getPropellerOnBench() == null){
					MTS.proxy.openGUI(BlockHelper.getTileEntityFromCoords(world, x, y, z), player);
				}
			}
		}
		return true;
