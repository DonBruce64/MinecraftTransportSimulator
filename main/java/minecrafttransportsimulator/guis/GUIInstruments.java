...
import minecrafttransportsimulator.mcinterface.MTSRendererInterface;
...
public class GUIInstruments extends MTSGui{
	...
	protected void handleDraw(int mouseX, int mouseY){
		...
		MTSRendererInterface.enableLightmap();
		GL11.glPopMatrix();
		...
		for(Entry<String, Integer[]> renderedInstrumentsEntry : renderedPlayerInstrumentsBounds.entrySet()){
			...
			MTSRendererInterface.renderItem(playerInstruments.get(renderedInstrumentsEntry.getKey()), 0, 0);
			GL11.glPopMatrix();
		}
		...
		MTSRendererInterface.drawText(I18n.format("gui.instruments.main"), (int) (-MTSRendererInterface.getTextWidth(I18n.format("gui.instruments.main"))/2), 0, Color.WHITE, false, false);
		GL11.glPopMatrix();
		...
		if(vehicle.pack.general.type.equals("plane")){
			...
			GL11.glScalef(2.5F, 2.5F, 2.5F);
			MTSRendererInterface.drawText(I18n.format("gui.instruments.control"), (int) (-MTSRendererInterface.getTextWidth(I18n.format("gui.instruments.control"))/2), 0, Color.WHITE, false, false);
			GL11.glPopMatrix();
		}
		...
		if(lastInstrumentClicked == null){
			if(vehicle.ticksExisted%40 >= 20){
				...
				GL11.glScalef(2.5F, 2.5F, 2.5F);
				MTSRendererInterface.drawText(I18n.format("gui.instruments.idle"), (int) (-MTSRendererInterface.getTextWidth(I18n.format("gui.instruments.idle"))/2), 0, Color.WHITE, false, false);
				GL11.glPopMatrix();
			}
		}else{
			GL11.glPushMatrix();
			...
			GL11.glScalef(2.5F, 2.5F, 2.5F);
			MTSRendererInterface.drawText(I18n.format("gui.instruments.decide"), (int) (-MTSRendererInterface.getTextWidth(I18n.format("gui.instruments.decide"))/2), 0, Color.WHITE, false, false);
			GL11.glPopMatrix();
		}
		...
		GL11.glScalef(2.5F, 2.5F, 2.5F);
		MTSRendererInterface.drawText(I18n.format("gui.instruments.clear"), (int) (-MTSRendererInterface.getTextWidth(I18n.format("gui.instruments.clear"))/2), 0, Color.WHITE, false, false);
		GL11.glPopMatrix();
	...
	protected void handleMouseClick(int mouseX, int mouseY){
		...
		if(lastInstrumentClicked != null){
			...
			for(Entry<String, Integer[]> renderedInstrumentsEntry : renderedPlayerInstrumentsBounds.entrySet()){
				if(mouseX > renderedInstrumentsEntry.getValue()[0] && mouseX < renderedInstrumentsEntry.getValue()[1] && mouseY > renderedInstrumentsEntry.getValue()[2] && mouseY < renderedInstrumentsEntry.getValue()[3]){
					MTSNetwork.sendPacketToServer(new PacketVehicleCommonInstruments(vehicle, player.getID(), getIndexOfLastInstrumentClicked(), renderedInstrumentsEntry.getKey()));
			...
			if(mouseX > RenderHUD.screenDefaultX/4 - 30 && mouseX < RenderHUD.screenDefaultX/4 + 30 && mouseY > 11*RenderHUD.screenDefaultY/16 - 30 && mouseY < 11*RenderHUD.screenDefaultY/16 + 30){
				if(vehicle.getInstrumentInfoInSlot(getIndexOfLastInstrumentClicked()) != null){
					MTSNetwork.sendPacketToServer(new PacketVehicleCommonInstruments(vehicle, player.getID(), getIndexOfLastInstrumentClicked(), ""));
					lastInstrumentClicked = null;
					return;
				}
			}
