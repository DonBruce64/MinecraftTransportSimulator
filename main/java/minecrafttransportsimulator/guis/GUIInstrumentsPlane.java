...
public class GUIInstrumentsPlane extends GuiScreen{
    ...
    public void drawScreen(int mouseX, int mouseY, float renderPartialTicks){
		...
		if(plane.instruments.get((byte) 0) != null){
			...
			GL11DrawSystem.bindTexture(AircraftInstruments.gauges[plane.instruments.get((byte) 0).currentInstrument]);
		...
		for(byte i=1; i<4; ++i){
			if(plane.instruments.get(i) != null){
				GL11.glPushMatrix();
				GL11DrawSystem.bindTexture(AircraftInstruments.gauges[plane.instruments.get((byte) i).currentInstrument]);
		...
		if(plane.instruments.get((byte) 4) != null){
			GL11.glPushMatrix();
			GL11.glScalef(0.75F, 0.75F, 0.75F);
			GL11DrawSystem.bindTexture(AircraftInstruments.gauges[plane.instruments.get((byte) 4).currentInstrument]);
		...
		for(byte i=0; i<numberEngineBays; ++i){
	    	...
	    	GL11DrawSystem.bindTexture(AircraftInstruments.gauges[plane.instruments.get((byte) (i*10 + 10)).currentInstrument]);
	    	GL11DrawSystem.renderSquare(-60, 0, 0, -60, 0, 0, false);
	    	GL11DrawSystem.bindTexture(AircraftInstruments.gauges[plane.instruments.get((byte) (i*10 + 11)).currentInstrument]);
	    	GL11DrawSystem.renderSquare(0, 60, 0, -60, 0, 0, false);
	    	GL11DrawSystem.bindTexture(AircraftInstruments.gauges[plane.instruments.get((byte) (i*10 + 12)).currentInstrument]);
	    	GL11DrawSystem.renderSquare(-60, 0, 60, 0, 0, 0, false);
	    	GL11DrawSystem.bindTexture(AircraftInstruments.gauges[plane.instruments.get((byte) (i*10 + 13)).currentInstrument]);
		...
		if(lastInstrumentClicked == -1){
			...
			for(byte i=1; i<AircraftGauges.values().length; ++i){
				if(player.capabilities.isCreativeMode){
					...
					hasInstrument[i] = EntityHelper.getQtyOfItemPlayerHas(player, MTSRegistry.flightInstrument, i) > 0;
				}
			}
			for(byte i=1; i<AircraftGauges.values().length; ++i){
				if(hasInstrument[i]){
    ...
    protected void mouseClicked(int x, int y, int button){
		if(lastClickedX == 0 && lastClickedY == 0){
			...
			if(x >= width/2 && x<= width/2+height*0.2 && y >= height*0.7 && y <= height*0.9){
				if(plane.instruments.get(lastInstrumentClicked).currentInstrument != 0){
