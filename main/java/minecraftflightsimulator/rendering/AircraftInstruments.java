package minecraftflightsimulator.rendering;
...
public final class AircraftInstruments{
	private static final ResourceLocation instrumentTexture = new ResourceLocation("mfs", "textures/instruments_aircraft.png");
	...
	public static void drawFlyableInstrument(EntityVehicle vehicle, int x, int y, byte instrumentCode, boolean hud, byte engineNumber){
		...
		if(instrumentCode != -1){
			switch (AircraftGauges.values()[instrumentCode]){
				...
				case AIRSPEED: drawAirspeedIndicator(vehicle, x, y, lighted); break;
				case TURNCOORD: drawTurnCoordinator(vehicle, x, y, lighted); break;
				case TURNSLIP: drawTurnAndSlipIndicator(vehicle, x, y, lighted); break;
				case VERTICALSPEED: drawVerticalSpeedIndicator(vehicle, x, y, lighted); break;
				case LIFTRESERVE: drawLiftReserveIndicator(vehicle, x, y, lighted); break;
				case TRIM: drawTrimIndicator(vehicle, x, y, lighted); break;
				case DUMMY: break;
				case TACHOMETER: drawTachometer(vehicle, x, y, lighted, engineNumber); break;
				case FUELQTY: drawFuelGauge(vehicle, x, y, lighted); break;
				case FUELFLOW: drawFuelFlowGauge(vehicle, x, y, lighted, engineNumber); break;
				case ENGINETEMP: drawEngineTempGauge(vehicle, x, y, lighted, engineNumber); break;
				case OILPRESSURE: drawOilPressureGauge(vehicle, x, y, lighted, engineNumber); break;
				case ELECTRIC: drawElectricalGauge(vehicle, x, y, lighted); break;
			}
		}else{
			 drawGaugeBase(vehicle, x, y, lighted);
		}
		if(lighted){
	...
	public static void drawFlyableControl(EntityVehicle vehicle, int x, int y, AircraftControls type, boolean hud){
		if(!hud){
			GL11.glDepthMask(false);
			GL11.glDisable(GL11.GL_LIGHTING);
		}
		switch (type){
			case THROTTLE: drawThrottle(vehicle, x, y, hud); break;
			case BRAKE: drawParkingBrake(vehicle, x, y, hud); break;
			case FLAPS: drawFlapIndicator((EntityPlane) vehicle, x, y, hud); break;
		}
		if(!hud){
			GL11.glDepthMask(true);
        	GL11.glEnable(GL11.GL_LIGHTING);
	...
	private static void drawGaugeBase(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
		if(lighted){
			Minecraft.getMinecraft().entityRenderer.enableLightmap(0);
			GL11DrawSystem.bindTexture(instrumentTexture);
	...
	private static void drawAttitudeIndicator(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
		GL11.glPushMatrix();
		GL11DrawSystem.bindTexture(instrumentTexture);
		
		rotationHelper(centerX, centerY, -vehicle.rotationRoll);
		if(vehicle.rotationPitch >= 24){
		...
		}else if(vehicle.rotationPitch <= -24){
			...
			GL11DrawSystem.renderQuadUV(centerX-20, centerX+20, centerX+20, centerX-20, centerY+20, centerY+20, centerY-20, centerY-20, 0, 0, 0, 0, 0.34375 - vehicle.rotationPitch*0.00390625, 0.65625 - 0.00390625*vehicle.rotationPitch, 0.53125,  0.84375, false);
		}
	...
	private static void drawAltimeter(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
    	drawGaugeBase(vehicle, centerX, centerY, lighted);
	...
	private static void drawHeadingIndicator(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
        ...
        GL11.glPushMatrix();
        rotationHelper(centerX, centerY, -vehicle.rotationYaw);
	...
	private static void drawAirspeedIndicator(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
    	drawGaugeBase(vehicle, centerX, centerY, lighted);
    	...
    	drawLongPointer(centerX, centerY, (float) (30+7.5*vehicle.velocity*ConfigSystem.getDoubleConfig("PlaneSpeedFactor")*20), 35, 2);
    }
    
	private static void drawTurnCoordinator(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
		GL11DrawSystem.bindTexture(instrumentTexture);
		drawGaugeBase(vehicle, centerX, centerY, lighted);
		GL11.glPushMatrix();
    	...
    	float turn = Math.max(Math.min(((vehicle.rotationRoll - vehicle.prevRotationRoll)/10 + vehicle.rotationYaw - vehicle.prevRotationYaw)/0.15F*25F, 50), -50);
    	...
    	double slip = vehicle.sideVec.dot(vehicle.velocityVec);
	...
	private static void drawTurnAndSlipIndicator(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
		GL11DrawSystem.bindTexture(instrumentTexture);
		drawGaugeBase(vehicle, centerX, centerY, lighted);
		GL11.glPushMatrix();
    	...
    	float turn = Math.max(Math.min((vehicle.rotationYaw - vehicle.prevRotationYaw)/0.15F*25F, 50), -50);
	...
	private static void drawVerticalSpeedIndicator(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
		drawGaugeBase(vehicle, centerX, centerY, lighted);
    	...
    	drawLongPointer(centerX, centerY, (float) (-90+10.625*vehicle.motionY*20), 35, 2);
	}
	
	private static void drawLiftReserveIndicator(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
		drawGaugeBase(vehicle, centerX, centerY, lighted);
    	GL11.glPushMatrix();
        ...
        float angle = (float) Math.max(Math.min(vehicle.trackAngle*3 + 20, 35), -35);
	...
	private static void drawTrimIndicator(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
    	drawGaugeBase(vehicle, centerX, centerY, lighted);
    	...
    	if(vehicle instanceof EntityPlane){
    		drawLongPointer(centerX, centerY-10, 135F/20F*((EntityPlane) vehicle).rudderTrim/10F, 14, 2);
    		rotationHelper(centerX-12, centerY+8, 75F/10F*((EntityPlane) vehicle).aileronTrim/10F + 90);
    		...
    		rotationHelper(centerX-12, centerY+8, -(75F/10F*((EntityPlane) vehicle).aileronTrim/10F + 90));
    		drawLongPointer(centerX+12, centerY+8, 135F/15F*((EntityPlane) vehicle).elevatorTrim/10F + 90, 14, 2);
    	}
    }
	
	private static void drawTachometer(EntityVehicle vehicle, int centerX, int centerY, boolean lighted, byte engineNumber){
    	drawGaugeBase(vehicle, centerX, centerY, lighted);
    	drawScaledString("RPM", centerX*2-10, centerY*2+14, 0.5F);
    	if(engineNumber == -1){
    		int lowestMaxRPM = 9999;
    		for(byte i=0; i<vehicle.getNumberEngineBays(); ++i){
    			if(vehicle.getEngineByNumber(i) != null){
    				lowestMaxRPM = Math.min(lowestMaxRPM, vehicle.getEngineByNumber(i).maxRPM);
    			}
    		}
			...
			if(vehicle.getEngineByNumber(engineNumber) != null){
				drawDialColoring(centerX, centerY, -135+vehicle.getEngineByNumber(engineNumber).maxRPM/10, 165, 25, 4, new float[] {1, 0, 0});
			}
		}
        ...
        if(engineNumber == -1){
    		for(byte i=0; i<vehicle.getNumberEngineBays(); ++i){
    			if(vehicle.getEngineByNumber(i) != null){
	    			drawLongPointer(centerX, centerY, (float) (-135+vehicle.getEngineByNumber(i).RPM/10), 30, 3);
    			}else{
    		...
    		if(vehicle.getEngineByNumber(engineNumber) != null){
    			drawLongPointer(centerX, centerY, (float) (-135+vehicle.getEngineByNumber(engineNumber).RPM/10), 30, 3);
    		}else{
    			drawLongPointer(centerX, centerY, (float) (-135), 30, 3);
    		}
    	}
	...
	private static void drawFuelGauge(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
    	drawGaugeBase(vehicle, centerX, centerY, lighted);
	...
	private static void drawFuelFlowGauge(EntityVehicle vehicle, int centerX, int centerY, boolean lighted, byte engineNumber){
    	drawGaugeBase(vehicle, centerX, centerY, lighted);
        ...
        if(engineNumber == -1){
    		...
    		for(byte i=0; i<vehicle.getNumberEngineBays(); ++i){
    			if(vehicle.getEngineByNumber(i) != null){
    				totalFuelFlow += vehicle.getEngineByNumber(i).fuelFlow;
    			}
    		}
    		drawLongPointer(centerX, centerY, (float) (-135 + totalFuelFlow*20*60*60/1000), 30, 3);
    	}else{
    		if(vehicle.getEngineByNumber(engineNumber) != null){
    			drawLongPointer(centerX, centerY, (float) (-135 + vehicle.getEngineByNumber(engineNumber).fuelFlow*20*60*60/1000), 30, 3);
    		}else{
    			drawLongPointer(centerX, centerY, (float) (-135), 30, 3);
    		}
    	}
	...
	private static void drawEngineTempGauge(EntityVehicle vehicle, int centerX, int centerY, boolean lighted, byte engineNumber){
		drawGaugeBase(vehicle, centerX, centerY, lighted);
        ...
        if(engineNumber == -1){
    		for(byte i=0; i<vehicle.getNumberEngineBays(); ++i){
    			if(vehicle.getEngineByNumber(i) != null){
	    			drawLongPointer(centerX, centerY, (float) (-135 + ((vehicle.getEngineByNumber(i).temp*9F/5F + 32) - 50)*1.35), 30, 3);
    			}else{
    				drawLongPointer(centerX, centerY, (float) (-135), 30, 3);
    			}
    		}
    		...
    		if(vehicle.getEngineByNumber(engineNumber) != null){
    			drawLongPointer(centerX, centerY, (float) (-135 + ((vehicle.getEngineByNumber(engineNumber).temp*9F/5F + 32) - 50)*1.35), 30, 3);
    		}else{
    			drawLongPointer(centerX, centerY, (float) (-135), 30, 3);
    		}
    	}
	...
	private static void drawOilPressureGauge(EntityVehicle vehicle, int centerX, int centerY, boolean lighted, byte engineNumber){
		drawGaugeBase(vehicle, centerX, centerY, lighted);
        ...
        if(engineNumber == -1){
    		for(byte i=0; i<vehicle.getNumberEngineBays(); ++i){
    			if(vehicle.getEngineByNumber(i) != null){
	    			drawLongPointer(centerX, centerY, (float) (-135 + vehicle.getEngineByNumber(i).oilPressure*1.8F), 30, 3);
    			}else{
    				drawLongPointer(centerX, centerY, (float) (-135), 30, 3);
    			}
    		}
    		...
    		if(vehicle.getEngineByNumber(engineNumber) != null){
    			drawLongPointer(centerX, centerY, (float) (-135 + vehicle.getEngineByNumber(engineNumber).oilPressure*1.8F), 30, 3);
    		}else{
    			drawLongPointer(centerX, centerY, (float) (-135), 30, 3);
    		}
    	}
	...
	private static void drawElectricalGauge(EntityVehicle vehicle, int centerX, int centerY, boolean lighted){
		drawGaugeBase(vehicle, centerX, centerY, lighted);
    	...
    	drawLongPointer(centerX - 17, centerY, (float) (145 - 6.875F*vehicle.electricPower), 26, 2);
    	drawLongPointer(centerX + 17, centerY, (float) (-90 - 27.5F*Math.min(vehicle.electricFlow*100F, 2)), 26, 2);
	...
	private static void drawThrottle(EntityVehicle vehicle, int centerX, int centerY, boolean hud){		
		...
		if(!hud){
    		...
    		GL11DrawSystem.renderQuadUV(centerX-1.75, centerX-1.75, centerX-1.75, centerX-1.75, centerY-1.75, centerY-1.75, centerY+1.75, centerY+1.75, -7-vehicle.throttle/10F, 0, 0, -7-vehicle.throttle/10F, 0.640625, 0.734375, 0.890625, 0.984375, false);
    		GL11DrawSystem.renderQuadUV(centerX+1.75, centerX+1.75, centerX+1.75, centerX+1.75, centerY+1.75, centerY+1.75, centerY-1.75, centerY-1.75, -7-vehicle.throttle/10F, 0, 0, -7-vehicle.throttle/10F, 0.640625, 0.734375, 0.890625, 0.984375, false);
        	GL11DrawSystem.renderQuadUV(centerX+1.75, centerX+1.75, centerX-1.75, centerX-1.75, centerY-1.75, centerY-1.75, centerY-1.75, centerY-1.75, -7-vehicle.throttle/10F, 0, 0, -7-vehicle.throttle/10F, 0.640625, 0.734375, 0.890625, 0.984375, false);
        	GL11DrawSystem.renderQuadUV(centerX-1.75, centerX-1.75, centerX+1.75, centerX+1.75, centerY+1.75, centerY+1.75, centerY+1.75, centerY+1.75, -7-vehicle.throttle/10F, 0, 0, -7-vehicle.throttle/10F, 0.640625, 0.734375, 0.890625, 0.984375, false);
        	
        	GL11DrawSystem.renderSquareUV(centerX-7, centerX+7, centerY+7, centerY-7, -7-vehicle.throttle/10F, -7-vehicle.throttle/10F, 0.75, 0.875, 0.875, 1, true);
    	}else{
        	GL11DrawSystem.renderSquareUV(centerX-5.25, centerX+5.25, centerY+0.175, centerY-10.5, 0, 0, 0.75, 0.875, 0.875, 1, false);
        	GL11DrawSystem.renderSquareUV(centerX-1.75, centerX+1.75, centerY+7+vehicle.throttle/10, centerY-7, 0, 0, 0.640625, 0.734375, 0.890625, 0.984375, false);
        	GL11DrawSystem.renderSquareUV(centerX-7, centerX+7, centerY+7+vehicle.throttle/10F, centerY-7+vehicle.throttle/10F, 0, 0, 0.75, 0.875, 0.875, 1, false);
    	}
    }
