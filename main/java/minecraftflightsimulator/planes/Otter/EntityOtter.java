...
public class EntityOtter extends EntityPlane{
	...
	public void initParentContainerSlots(ContainerParent container){
		//TODO make new GUI
		container.addSlotToContainer(new SlotLandingGear(this, 86, 113, 1, MFS.proxy.wheelLarge));
		container.addSlotToContainer(new SlotLandingGear(this, 50, 113, 2, MFS.proxy.wheelLarge));
		container.addSlotToContainer(new SlotLandingGear(this, 68, 113, 4, MFS.proxy.wheelLarge));
		
		container.addSlotToContainer(new SlotPart(this, 131, 66, 6, MFS.proxy.engineSmall));
		container.addSlotToContainer(new SlotPart(this, 131, 82, 7, MFS.proxy.engineSmall));
		
		container.addSlotToContainer(new SlotPart(this, 150, 66, 10, MFS.proxy.propeller));
		container.addSlotToContainer(new SlotPart(this, 150, 82, 11, MFS.proxy.propeller));
		...
		for(int i=0; i<10; ++i){
			container.addSlotToContainer(new SlotPart(this, 7 + 18*(i%5), i < 5 ? 7 : 25, i + instrumentStartSlot, MFS.proxy.flightInstrument));
		}
	}
}