...
public class InterfaceClient implements IInterfaceClient {
    ...
    public WrapperPlayer getClientPlayer() {
        return (WrapperPlayer) WrapperPlayer.getWrapperFor(Minecraft.getInstance().player);
    }
        ...
        if (!InterfaceManager.clientInterface.isGamePaused() && player != null) {
            ...
            if (world != null) {
                if (event.phase.equals(Phase.START)) {
                    ...
                    if (actuallyFirstPerson ^ Minecraft.getInstance().options.getCameraType() == CameraType.FIRST_PERSON) {
                        changedCameraState = true;
                        actuallyFirstPerson = Minecraft.getInstance().options.getCameraType() == CameraType.FIRST_PERSON;
                    }
                    if (actuallyThirdPerson ^ Minecraft.getInstance().options.getCameraType() == CameraType.THIRD_PERSON_BACK) {
                        changedCameraState = true;
                        actuallyThirdPerson = Minecraft.getInstance().options.getCameraType() == CameraType.THIRD_PERSON_BACK;
                    }
