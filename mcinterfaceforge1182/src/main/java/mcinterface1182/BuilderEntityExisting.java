    ...
    public void baseTick() {
        ...
        if (entity != null) {
            ...
            if (!entity.isValid) {
                ...
                if (!level.isClientSide && position().y < 0 && Level.isOutsideBuildHeight(blockPosition())) {
                    remove();
                    return;
                }
        ...
        if (ConfigSystem.settings.damage.allowExternalDamage.value && !level.isClientSide && entity instanceof AEntityE_Interactable) {
            ...
            if (lastExplosionPosition != null && source.isExplosion()) {
                ...
                if (damage == null) {
                    ...
                    Optional<Vec3> hitRaytrace = interactionBoxes.clip(attackerMcPos, attackerMcPos.add(attacker.getDeltaMovement()));
                    if (hitRaytrace != null) {
