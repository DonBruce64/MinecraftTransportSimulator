...
public class WrapperWorld extends AWrapperWorld {
     ...
     */
    public static WrapperWorld getWrapperFor(ClientLevel world) {
        if (world != null) {
    ...
    public BlockHitResult getBlockHit(Point3D position, Point3D delta) {
        ...
        BlockHitResult trace = world.clip(new RayTraceContext(start, start.add(delta.x, delta.y, delta.z), RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.NONE, null));
            ...
            for (int j = (int) Math.floor(mcBox.minY); j < Math.ceil(mcBox.maxY); ++j) {
                for (int k = (int) Math.floor(mcBox.minZ); k < Math.ceil(mcBox.maxZ); ++k) {
                    ...
                    if (!world.isEmptyBlock(pos)) {
                        ...
                        if (!collisionShape.isEmpty() && Shapes.joinIsNotEmpty(mcShape, collisionShape, IBooleanFunction.AND)) {
    ...
    public <BlockEntityType extends ATileEntityBase<JSONDefinition>, JSONDefinition extends AJSONMultiModelProvider> boolean setBlock(ABlockBase block, Point3D position, IWrapperPlayer playerWrapper, Axis axis) {
        if (!world.isClientSide) {
    ...
    public boolean placeBlock(Point3D position, IWrapperItemStack stack) {
        ...
        if (world.isEmptyBlock(pos)) {
            ...
            if (mcStack.useOn(new UseOnContext(world, null, InteractionHand.MAIN_HAND, mcStack, new BlockHitResult(new Vec3(position.x, position.y, position.z), Direction.DOWN, pos, true))) == InteractionResult.CONSUME) {
                return true;
            }
        }
        return false;
