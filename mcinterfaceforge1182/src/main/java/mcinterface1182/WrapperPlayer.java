...
public class WrapperPlayer extends WrapperEntity implements IWrapperPlayer {
    ...
    public void displayChatMessage(LanguageEntry language, Object... args) {
        Minecraft.getInstance().gui.getChat().addMessage(new TextComponent(String.format(language.value, args)));
    }
    ...
    public IWrapperEntity getLeashedEntity() {
        for (MobEntity mobEntity : player.level.getLoadedEntitiesOfClass(MobEntity.class, new AABB(player.position().x - 7.0D, player.position().y - 7.0D, player.position().z - 7.0D, player.position().x + 7.0D, player.position().y + 7.0D, player.position().z + 7.0D))) {
