        ...
        DamageSource newSource = new EntityDamageSource(damage.language.value, damage.entityResponsible != null ? ((WrapperEntity) damage.entityResponsible).entity : null) {
            ...
            public Component getLocalizedDeathMessage(LivingEntity player) {
                if (damage.entityResponsible != null) {
                    return new TextComponent(String.format(damage.language.value, player.getDisplayName().getString(), ((WrapperEntity) damage.entityResponsible).entity.getDisplayName().getString()));
                } else {
                    return new TextComponent(String.format(damage.language.value, player.getDisplayName().getString()));
                }
            }
        };
        if (damage.isFire) {
